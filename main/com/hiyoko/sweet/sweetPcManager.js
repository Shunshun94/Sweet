...
com.hiyoko.sweet.PcManager.prototype.buildComponent = function() {
	...
	this.console = new com.hiyoko.sweet.PcManager.Console(this.getElementById('manager'));
};
...
com.hiyoko.sweet.PcManager.prototype.bindEvents = function() {
	...
	this.$html.on(com.hiyoko.sweet.PcManager.Prepare.EVENTS.EXEC, function(e) {
	...
	this.$html.on(com.hiyoko.sweet.PcManager.Console.EVENTS.UPDATE, function(e) {
...
com.hiyoko.sweet.PcManager.Prepare.prototype.onExec = function(e) {
	var list = [];
	this.getElementsByClass('id').each(function(i, v) {
		list.push($(this).val());
	});
	this.fireEvent(new $.Event(com.hiyoko.sweet.PcManager.Prepare.EVENTS.EXEC, {sheets: list.filter(function(v){return Number(v)})}));
};

com.hiyoko.sweet.PcManager.Prepare.prototype.onInputBoxChange = function(e) {
	var $target = $(e.target);
	var text = $target.val();
...
com.hiyoko.sweet.PcManager.Console = function($html) {
	this.$html = $html;
	this.id = $html.attr('id');
	this.sheets = [];
	...
	this.getElementById('update').click(function(e) {
		this.fireEvent(new $.Event(com.hiyoko.sweet.PcManager.Console.EVENTS.UPDATE, {sheets: this.sheets}));
	}.bind(this));
	this.disable();
};
com.hiyoko.util.extend(com.hiyoko.component.ApplicationBase, com.hiyoko.sweet.PcManager.Console);

com.hiyoko.sweet.PcManager.Console.prototype.generateCharacterDom = function(character) {
	...
	$base.css(
			'background-color',
			io.github.shunshun94.util.Color.HslToRgb((Number(character.id) % 360), 100, 5).code);
	$base.append(com.hiyoko.util.format('<span class="%s-name">%s</span><br/>', baseClass, character.name));
	var $hp = $(com.hiyoko.util.format('<span class="%s-hp">H %s / %s</span>', baseClass, character.hp, character.mhp));
	$hp.css('color', this.colorTool.getColor(100 * Number(character.hp) / Number(character.mhp)).code);
	$base.append($hp);
	$base.append(com.hiyoko.util.format('<br/><span class="%s-mp">M %s / %s</span><br/>', baseClass, character.mp, character.mmp));
...
com.hiyoko.sweet.PcManager.Console.prototype.insertCharacters = function(characters) {
...
com.hiyoko.sweet.PcManager.Console.EVENTS = {
